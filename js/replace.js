    ...
    var dictionary =
     {
       ...
       'ソケット'        : 'カーネル空間のネットワーク関連の処理にアクセスするためのインターフェース。具体的には、C++ファイル内ににおいてsock = socket(AF_INET, SOCK_STREAM, htons(ETH_P_IP));　のようにすることで、NICから入ってきたパケットにアクセスするための手法として使う。AF_INETのところをPF_PACKETとすると届いたパケットをコピーする処理になるため、Clickでは恐らくこれを使っているのではないか。',
